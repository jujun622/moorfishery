...
from .policy.GA import GA, GA2
from .policy.CA import CA
...
def train_policy(model, seed=345, MaxTry=5, UpdateTime=15, PopulationSize=50, GenerationTime=10, search_time=5, p_c=0.5, p_m=1, 
                gen_method='SCO', reset_mode='uniform', random_state=42):
    # policy learning
    np.random.seed(seed)
    ...
    if gen_method == 'SCO':
        ...
        best = GA(f, thd_num=len(A) - 1, K=K, PopulationSize=PopulationSize, GenerationTime=GenerationTime, p_c=p_c, p_m=p_m)
    elif gen_method == 'CA':
...
def evaluate_tp(simulator, policy, world, reset_world_mode = 'uniform',
    ...
    return policy_eval_rslt


def expt_tp(model_params_list, hyperparameters, path, despot, reset_world_mode='uniform', m=2, random_state=42):
    ...
    for p in range(M):
        ...
        print(np.array(policies[p].thresholds))
        # policies[p].normalize()
        # print('Relative threshold policy:')
        # print(policies[p])
        # print('')
    ...
    for m in range(M-1, M):     #only evaluate on the GT world model
        ...
        for p in range(M):
            ...
            if m == p:
                ...
                tmp = evaluate_tp(**restrict(params, evaluate_tp))
                vs.append(tmp)

                print('evaluation by threshold policy:', tmp)

        values.append(vs)

    # index = ['$M_{%s}$' % (model_params_list[m]['ID']) for m in range(M)]
    index = ['$M_{%s}$' % (model_params_list[m]['ID']) for m in range(M-1, M)]
    # columns = np.array([['$\pi_{%s}^{n}$' % (model_params_list[m]['ID']),
    #                      '$\pi_{%s}^{un}$' % (model_params_list[m]['ID'])] for m in range(M)]).flatten()
    # columns = np.array([['$\pi_{%s}$' % (model_params_list[m]['ID'])] for m in range(M)]).flatten()
