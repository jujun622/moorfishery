...
def expt(model_params_list, hyperparameters, path, despot, reset_world_mode='uniform', m=2, random_state=42):
    ...
    for params in model_params_list:
        ...
        # run despot to get the optimal solution
        # print('')
        # print('***Running DESPOT now...')
        # t = hyperparameters['despot_t']
        # runs = hyperparameters['runs'] #1000
        # simlen = hyperparameters['simlen']  #100

        # despot_rslt = model.get_despot_result(t, runs, simlen, despot_path, despot, info=params['ID'])
        # despot_results.append(despot_rslt)
    ...
    for m in range(M-1, M):     #only evaluate on the GT world model
        ...
        for p in range(M):
            ...
            if m == p:
                ...
                tmp = evaluate(**restrict(params, evaluate))
                vs.append(tmp[0])
                vs.append(tmp[1])

                print('evaluation by threshold policy:', tmp[0])
                print('evaluation by despot:', tmp[1])
    ...
    for i in range(6):
        if i%2 == 0:
            ...
            columns.append('DESPOT (Sim:%s)' % (model_params_list[int(i/2)]['ID']))
